...
export function rootDir(): Directory {
    ...
    if (!neeva.translations) {
        throw new Error('NEEVA_TRANSLATIONS_REPO is not set');
    }

    return Directory.fromPath(neeva.translations);
}
...
export async function init(): Promise<void> {
    ...
    if (!isInitialized) {
        ...
        await git().cwd(neeva.translations);
        isInitialized = true;
    }
}
